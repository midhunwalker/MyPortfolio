...
const NebulaBackground = () => {
  ...
  const driftRef = useRef({ x: 0, y: 0, dx: 0.05, dy: 0.03 }); // diagonal drift
  ...
  useEffect(() => {
    ...
    function initStars() {
      ...
      for (let i = 0; i < numStars; i++) {
        ...
        stars.push({
          ...
          radius: (layer === "foreground" ? Math.random() * 1.8 + 0.7 : Math.random() * 1.2) * 3,
    ...
    function animateStars() {
      ...
      // update diagonal drift
      driftRef.current.x += driftRef.current.dx;
      driftRef.current.y += driftRef.current.dy;
      if (driftRef.current.x > 20 || driftRef.current.x < -20) driftRef.current.dx *= -1;
      if (driftRef.current.y > 15 || driftRef.current.y < -15) driftRef.current.dy *= -1;
      ...
      starsRef.current.forEach((star) => {
        ...
        let drawY = star.y + scrollOffset * parallaxFactor + driftRef.current.y * parallaxFactor;
